...
(function() {
    ...
    const WORKER_URL = "https://whitelabel-api.learnaiwithnik.workers.dev"; // UPDATE THIS
    
    // AGENCY BRANDING
    const BRAND_COLOR = window.AIFL_BTN_COLOR || "#000";
    const AGENCY_NAME = window.AIFL_AGENCY_NAME || "Virtual Try-On";
    ...
    const injectStyles = () => {
        ...
        s.innerHTML = `
            ...
            .v-brand-logo { font-size:11px; font-weight:800; letter-spacing:1px; margin-bottom:15px; text-transform:uppercase; color: #555; display: inline-block; }
    ...
    const checkPage = () => {
        ...
        // Does the owner have the "vton" tag anywhere? (URL or Page Text)
        const hasVtonTag = fullUrl.includes('vton') || pageText.includes('vton');
        
        // Show if Product Page AND has VTON Tag
        if ((isProductUrl && hasVtonTag) || (window.VTON_CONFIG && window.VTON_CONFIG.force)) {
            showButton();
        } else {
            removeButton();
        }
    ...
    function showGuide() {
        ...
        let trials = localStorage.getItem("vton_trials") || 3;
        ...
        ov.innerHTML = `
            <div class="v-con">
                ...
                <div class="v-right">
                    <div class="v-brand-logo">POWERED BY ${AGENCY_NAME}</div>
            ...
            <div class="v-con">
                ...
                <div class="v-right">
                    ...
                    <div style="font-size:10px; color:#999; margin-top:20px; text-align:center; line-height: 1.4;">
                        🛡️ Privacy Note: Images are processed in real-time and never stored by ${AGENCY_NAME}.
                    </div>
                </div>
            </div>
        `;
